...
require_once( dirname( __FILE__ ) . '/class-post-h2-adverts-base.php' );
...
class iworks_post_h2_adverts extends iworks_post_h2_adverts_base {
	...
	public function admin_init() {
		iworks_post_h2_adverts_options_init();
	...
	public function admin_enqueue_scripts() {
		...
		/**
		 * off on not post-h2-adverts pages
		 */
		...
		$file    = plugins_url( $file, $this->base );
		wp_register_style( 'admin-post-h2-adverts', $file, array( 'jquery-ui-datepicker', 'select2' ), $version );
		wp_enqueue_style( 'admin-post-h2-adverts' );
		/**
		 * select2
		 */
		 ...
		 */
		$files = array(
			'post-h2-adverts-admin' => sprintf( 'assets/scripts/admin/admin%s.js', $this->dev ),
		);
		if ( '' == $this->dev ) {
			$files = array(
				'post-h2-adverts-admin-datepicker' => 'assets/scripts/admin/src/datepicker.js',
				'post-h2-adverts-admin-select2'    => 'assets/scripts/admin/src/select2.js',
			);
		}
		$deps = array(
		...
		wp_localize_script(
			'post_h2_adverts_admin',
			__CLASS__,
			apply_filters( 'wp_localize_script_post_h2_adverts_admin', $data )
		);
	}
	...
	public function init() {
		if ( is_admin() ) {
			...
			$file = 'assets/styles/post_h2_adverts' . $this->dev . '.css';
			wp_enqueue_style( 'post-h2-adverts', plugins_url( $file, $this->base ), array(), $this->get_version( $file ) );
		}
	}
	...
	public function plugin_row_meta( $links, $file ) {
		if ( $this->dir . '/post-h2-adverts.php' == $file ) {
			if ( ! is_multisite() && current_user_can( $this->capability ) ) {
				$links[] = '<a href="admin.php?page=' . $this->dir . '/admin/index.php">' . __( 'Settings' ) . '</a>';
			}
			/* start:free */
			$links[] = '<a href="http://iworks.pl/donate/post-h2-adverts.php">' . __( 'Donate' ) . '</a>';
			/* end:free */
		}
		return $links;
	}
